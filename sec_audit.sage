...
# 1. Divide by zero (should report error, not crash)
test("10 PRINT 1/0", "divide by zero")

# 2. Infinite GOTO loop (should hit step guard, not OOM)
test("10 GOTO 10", "infinite GOTO")

# 3. Invalid GOTO label
test("10 GOTO 99999", "invalid GOTO label")

# 4. Deep FOR loop
test("10 FOR I=1 TO 1000: PRINT I: NEXT I", "deep FOR loop")

# 5. POKE to device address ($2000 = UART status)
test("10 POKE 2000, 65", "POKE to UART status register")

# 6. CALL to monitor entry (-151)
test("CALL -151", "CALL -151 -> monitor")

# 7. Very long line (tokenizer stress)
...
# 9. Control characters in string
test("10 PRINT CHR$(27)", "ESC char in PRINT")

# 10. BSAVE with huge length (DoS vector)
test("BSAVE TEST,A0,L999999999", "BSAVE huge length")

# 11. DOS catalog with weird name
test("SAVE \"X,Y\"", "comma in name")

# 12. Unterminated quote in INPUT (known parser issue)
test("10 INPUT \"unterminated", "unterminated quote in INPUT")

# 13. BLOAD to device register
test("BLOAD TEST,A2000", "BLOAD to UART")

# 14. Monitor dump with huge range
test("800.FFFFFFFF", "monitor huge dump range")

# 15. Self-referencing EXEC (infinite recursion)
