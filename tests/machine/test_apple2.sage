...
print("== DOS text buffers and EXEC ==")
os.command("append BUF1")
os.drain()
os.command("close BUF1")
os.drain()
os.command("append BUF2")
os.drain()
os.command("open BUF1")
os.drain()
os.command("close BUF1")
os.drain()
os.command("read 2")
check(not contains(os.out, "FILE NOT OPEN"), "buffer numbers remain stable after close")
os.command("open MISSING")
check(contains(os.out, "FILE NOT FOUND"), "OPEN rejects missing files")
os.command("append RECUR")
os.drain()
os.command("close RECUR")
os.drain()
check(m.bus.storage.save_text("RECUR", ["EXEC RECUR"]) == 0, "recursive EXEC file created")
os.drain()
os.command("exec RECUR")
check(contains(os.out, "EXEC DEPTH EXCEEDED"), "EXEC recursion is bounded")
os.drain()
...
os.command("append BIN1")
check(contains(os.out, "FILE TYPE MISMATCH"), "APPEND refuses binary files")
...
os.drain()
os.command("10 POKE 8192,0")
os.command("20 END")
os.drain()
os.command("RUN")
check(contains(os.out, "ILLEGAL QUANTITY ERROR"), "POKE rejects device addresses")
os.drain()
