...
b.reset()
let video = apple2bus.Apple2Bus()
video.reset()
check(video.video_snapshot() == [true, false, false, false], "video state resets to text page one lores")
check(video.video_mode() == "text" and video.video_page() == 1, "video mode and page expose reset defaults")
video.write8(0xC050, 0x10)
check(video.video_snapshot() == [false, false, false, false], "C050 write turns text off")
video.write8(0xC051, 0x11)
check(video.video_snapshot() == [true, false, false, false], "C051 write turns text on")
video.write8(0xC052, 0x12)
check(video.video_snapshot() == [true, false, false, false], "C052 write turns mixed off")
video.write8(0xC053, 0x13)
check(video.video_snapshot() == [true, true, false, false] and video.video_mode() == "text", "C053 exposes mixed while text remains active")
video.write8(0xC054, 0x14)
check(video.video_snapshot() == [true, true, false, false], "C054 write selects page one")
video.write8(0xC055, 0x15)
check(video.video_snapshot() == [true, true, true, false], "C055 write selects page two")
video.write8(0xC056, 0x16)
check(video.video_snapshot() == [true, true, true, false], "C056 write selects lores")
video.write8(0xC057, 0x17)
check(video.video_snapshot() == [true, true, true, true], "C057 write selects hires")
check(video.video_events == [[0xC050, 0x10], [0xC051, 0x11], [0xC052, 0x12], [0xC053, 0x13], [0xC054, 0x14], [0xC055, 0x15], [0xC056, 0x16], [0xC057, 0x17]], "video writes retain ordered latch events")
video.write8(0xC054, 0x00)
video.write8(0xC056, 0x00)
video.write8(0xC050, 0x00)
check(video.video_mode() == "lores" and video.video_page() == 1, "video mode and page expose lores page one")
video.write8(0xC057, 0x00)
check(video.video_mode() == "hires" and video.video_page() == 1, "video mode and page expose hires page one")
video.write8(0xC051, 0x00)
check(video.video_mode() == "text" and video.video_page() == 1, "video mode and page expose text page one")
video.write8(0xC055, 0x00)
check(video.video_page() == 2, "video page two remains observable")
video.write8(0xC050, 0x00)
check(video.video_page() == 2 and video.video_mode() == "hires", "video page persists across mode changes")
video.write8(0xC051, 0x00)
check(video.video_page() == 2 and video.video_mode() == "text", "text mode preserves page two")
let video_read = apple2bus.Apple2Bus()
let read_addresses = [0xC050, 0xC051, 0xC052, 0xC053, 0xC054, 0xC055, 0xC056, 0xC057]
let read_snapshots = [[false, true, true, true], [true, true, true, true], [true, false, true, true], [true, true, true, true], [true, true, false, true], [true, true, true, true], [true, true, true, false], [true, true, true, true]]
var read_index = 0
while read_index < 8:
    video_read.write8(read_addresses[read_index], 0xA0 + read_index)
    read_index = read_index + 1
check(len(video_read.video_events) == 8, "video read setup retains eight write events")
read_index = 0
while read_index < 8:
    check(video_read.read8(read_addresses[read_index]) == 0x80 and video_read.video_snapshot() == read_snapshots[read_index], "video read switch " + str(read_index) + " updates state and preserves latch")
    read_index = read_index + 1
check(len(video_read.video_events) == 8, "video reads do not append write events")
let video_unlatched = apple2bus.Apple2Bus()
check(video_unlatched.read8(0xC051) == 0x00 and video_unlatched.video_snapshot() == [true, false, false, false], "video read retains unlatched deterministic return")
let state_before_clear = video_read.video_snapshot()
let mode_before_clear = video_read.video_mode()
let page_before_clear = video_read.video_page()
video_read.clear_events()
check(len(video_read.video_events) == 0 and video_read.video_snapshot() == state_before_clear and video_read.video_mode() == mode_before_clear and video_read.video_page() == page_before_clear and video_read.video_state(0) and video_read.video_value(0) == 0xA0, "clear_events preserves canonical video state and latches")
video_read.reset()
check(video_read.video_snapshot() == [true, false, false, false] and video_read.video_mode() == "text" and video_read.video_page() == 1 and len(video_read.video_events) == 0 and not video_read.video_state(0) and video_read.video_value(0) == 0, "reset clears canonical video state and latches")
...
check(b.video_events == [[0xC052, 0x5A]], "video write event remains compatible")
check(not b.video_state(8) and b.video_value(8) == 0, "legacy video accessors remain bounded")
