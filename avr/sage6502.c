...
static uint32_t cycount;        /* instruction cycle counter (parity with cpu.sage) */
static volatile uint8_t irq_pending, nmi_pending;

/* set by fetch() when an indexed read crosses a page boundary */
static uint8_t fetch_crossed = 0;
...
/* effective address for non-immediate memory modes; mode==0 (imm) handled
   by the caller, returns addr (pc already advanced); fetch_crossed is set
   for abs,X abs,Y (zp),Y when the effective address crosses a page */
static uint16_t fetch(uint8_t mode) {
    ...
    case M_ABSX: {
        ...
        uint16_t eff = (uint16_t)((base + rX) & 0xFFFF);
        if ((base & 0xFF00) != (eff & 0xFF00)) fetch_crossed = 1;
        return eff;
    }
    case M_ABSY: {
        uint8_t lo = bus_read(rPC);
        ...
        rPC = (rPC + 2) & 0xFFFF;
        uint16_t base = (uint16_t)((hi << 8) | lo);
        uint16_t eff = (uint16_t)((base + rY) & 0xFFFF);
        if ((base & 0xFF00) != (eff & 0xFF00)) fetch_crossed = 1;
        return eff;
    }
    case M_INDX: {
    ...
    case M_IND: {
        ...
        /* real 6502 bug: JMP ($xxFF) reads the high byte back on the same
           page ($xx00), not from the next page */
        return (uint16_t)(bus_read(ptr) | (bus_read((uint16_t)((ptr & 0xFF00) | ((ptr + 1) & 0xFF))) << 8));
    }
    }
    return 0;
}
...
/* branch with extra cycles: 1 taken, 2 taken + page crossing */
static uint8_t branch(uint8_t id) {
    int8_t off = sbyte(bus_read(rPC));
    rPC = (rPC + 1) & 0xFFFF;
    uint16_t srcpage = rPC & 0xFF00;
    uint8_t take = 0;
    switch (id) {
    ...
    if (take) {
        rPC = (uint16_t)((rPC + off) & 0xFFFF);
        if ((rPC & 0xFF00) != srcpage) return 2;
        return 1;
    }
    return 0;
}
...
/* decode one instruction; returns extra cycles (page-cross / branch) */
static uint8_t exec_step(uint8_t code) {
    uint8_t extra = 0;
    rPC = (rPC + 1) & 0xFFFF;
#ifdef HOST
...
#ifdef HOST
    uint8_t base = CY[code];
#else
    uint8_t base = pgm_read_byte(&CY[code]);
#endif
    cycount += (uint32_t)base + exec_step(code);
    /* service interrupts between instructions */
    if (nmi_pending) {
        nmi_pending = 0;
        serve_interrupt(0xFFFA);
    } else if (irq_pending && !F_I()) {
        irq_pending = 0;
        serve_interrupt(0xFFFE);
    }
}
uint8_t cpu_halted(void) { return halted; }

/* soft interrupt latches (parity with cpu.sage interrupt()/nmi()) */
void cpu_interrupt(void) { irq_pending = 1; }
void cpu_nmi(void) { nmi_pending = 1; }
...
uint32_t cpu_cycle_count(void) { return cycount; }
